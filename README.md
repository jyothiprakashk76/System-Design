...
| 💬 | **[Building with AI Coding Agents](LinkedIn_AI_Agents_Carousel/)** | *LinkedIn carousel, not a reference sheet.* Fourteen slides at 4:5 for the mobile feed: five failure modes of agentic coding — confident wrongness, review collapse, the 80% trap, silent scope drift, tests that prove nothing — and eight working practices, closing on the shift from author to reviewer. |
| 📈 | **[Engineering Leadership KPIs](Engineering_Leadership_KPIs/)** | What a Director or Head of Engineering measures: delivery, reliability, cost and unit economics, security, tech health and people as universal KPIs, then domain-specific sets for e-commerce, streaming, social platforms, chat, video calling and news feeds — plus the four-layer measurement model, counter-metrics for every KPI, what never to measure, and an operating cadence. |
| 🐳 | **[Docker & Containers — Architecture, Operations & Reference](Docker_Reference/)** | What a container actually is, images and layers with the build cache, Dockerfile instructions, the client and daemon with both lifecycles, building well with multi-stage and BuildKit, base images and size, networking, volumes, Compose, runtime and resources, security, registries and the supply chain, debugging, and production practice with CI/CD. |
| ☸️ | **[Kubernetes — Architecture, Operations & Reference](Kubernetes_Reference/)** | The reconciliation model and when not to use it, workload objects, config and secrets, the control plane and nodes with the path from manifest to running container, networking with Ingress versus Gateway API, storage, resources and scheduling, autoscaling, probes and lifecycle, security, observability, delivery and rollouts, multi-tenancy, and operations covering upgrades, cost and disaster recovery. |
| 🌬️ | **[Apache Airflow — Architecture, Operations & Reference](Airflow_Reference/)** | Orchestrate rather than compute, core concepts and the data interval model, scheduling and trigger rules, the components and how a task actually gets run, writing DAGs with TaskFlow and dynamic mapping, executors, deployment and DAG delivery, configuration, the four concurrency limits and pools, XCom and connections, sensors and deferrable operators, operations and the 2 to 3 upgrade, security, and monitoring with testing and CI/CD. |
| 🆚 | **[Data Lake vs Warehouse vs Lakehouse](Lake_Warehouse_Lakehouse/)** | Three eras of the same problem rather than three competing products: what each solved and what forced the next, the three architectures side by side, a comparison corrected for where things actually stand, the enabling technical shifts, open table formats where they genuinely differ, the catalog, choosing, cost models, migration paths, what stays the same across all three, and the operational reality. |
| 🏛️ | **[Enterprise Data Platform — Architecture, Operations & Reference](Enterprise_Data_Platform/)** | The layers and the contract each owes the next, batch versus streaming, the reference flow with its cross-cutting concerns, file formats versus table formats versus the catalog, schema change and data contracts, transformation and modelling, table maintenance, orchestration, data quality and observability, governance and privacy, serving and consumption, cost, and the operating model with a build order. |
| 💍 | **[Apache Cassandra — Architecture, Operations & Reference](Cassandra_Reference/)** | Query-first data modelling and the partition key decision, anti-patterns, the token ring and replication with both request paths, consistency levels and R + W > RF, CQL query rules, the storage engine and read amplification, compaction strategies, tombstones and repair, lightweight transactions and batches, indexes and views, node lifecycle operations, security and backup, and production failures. |
| 🐘 | **[PostgreSQL — Architecture, Operations & Reference](PostgreSQL_Reference/)** | MVCC and everything downstream of it — bloat, vacuum and transaction ID wraparound; isolation levels and write skew; processes, shared memory and the WAL; indexing and query planning; replication and the slot that fills your disk; connection pooling; data types and JSONB; partitioning; locking and safe migrations; memory configuration; extensions and observability. |
| 🔍 | **[Elasticsearch — Architecture, Operations & Reference](Elasticsearch_Reference/)** | The inverted index and why it is not a system of record, mappings and the text-versus-keyword decision, text analysis, nodes and shards and segments with both request paths, indexing performance, shard sizing, querying and relevance, aggregations and their approximations, cluster HA, lifecycle and data tiers, security, JVM heap and circuit breakers, snapshots, and production failures. |
